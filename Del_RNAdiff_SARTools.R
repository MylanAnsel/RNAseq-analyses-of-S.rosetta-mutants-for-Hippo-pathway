...
workDir <- "W:/mansel/RNAseq/For_publication/KO_Deletion/output_R_analyses"      # working directory for the R session

projectName <- "Chantal2025_KO_Deletion"           # name of the project
author <- "Ansel Mylan"                                 # author of the statistical analysis/report

targetFile <- "W:/mansel/RNAseq/For_publication/KO_Deletion/input_for_R_analysis/design_clone_combined.txt"       # path to the design/target file
rawDir <- "W:/mansel/RNAseq/For_publication/KO_Deletion/input_for_R_analysis/all_feature_counts"                  # path to the directory containing raw counts files
featuresToRemove <- NULL # NULL if no feature to remove

varInt <- "genotype"                                 # factor of interest
condRef <- "Sr"                                      # reference biological condition
...
saveRDS(out.DESeq2$dds, file = "W:/mansel/RNAseq/For_publication/KO_Deletion/output_R_analyses/Del_out_deseq2_dds.rds")
...
dds_full <- DESeq(dds_full, test = "LRT", reduced = ~ genotype)
...
exploreCounts_1(object = out.DESeq2$dds,
                ...
                col = colors,
                save_pca_svg = TRUE,
                pca_svg_filename = "W:/mansel/RNAseq/For_publication/KO_Deletion/PCA1-2.svg",
